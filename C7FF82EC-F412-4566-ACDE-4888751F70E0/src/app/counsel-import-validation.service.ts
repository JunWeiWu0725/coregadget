...
export class CounselImportValidationService {
  ...
  private validators: Record<string, ValidatorFn[]> = {
    ...
    學號: [
      ...
      (v) =>
        !/^[A-Z0-9]{4,10}$/.test(String(v).trim())
          ? "學號格式錯誤（4~10位英數字）"
          : null,
    ],
    學年度: [
      ...
      (v) =>
        Number(v) > new Date().getFullYear() - 1911 ? "學年度不能是未來" : null,
    ],
    學期: [
      (v) => (this.isBlank(v) ? "學期為必填" : null),
      (v) =>
        this.allowedValues["學期"].indexOf(String(v)) === -1
          ? "學期應為 1 或 2"
          : null,
    ],
    日期: [
    ...
    方式: [
      ...
      (v) =>
        this.allowedValues["方式"].indexOf(String(v).trim()) === -1
          ? "方式應為 " + this.allowedValues["方式"].join("、")
          : null,
    ],
    對象: [
      (v) => (this.isBlank(v) ? "對象為必填" : null),
      (v) =>
        this.allowedValues["對象"].indexOf(String(v).trim()) === -1
          ? "對象應為 " + this.allowedValues["對象"].join("、")
          : null,
    ],
    記錄者: [
      (v) => (this.isBlank(v) ? "記錄者為必填" : null),
      (v) =>
        !/^[\u4e00-\u9fa5]{2,}[\u4e00-\u9fa5（）()]+$/.test(String(v).trim())
          ? "記錄者應為教師姓名"
          : null,
    ],
    公開: [
      ...
      (v) =>
        this.allowedValues["公開"].indexOf(String(v).trim()) === -1
          ? "公開欄位應為 是 或 否"
          : null,
    ],
    類別: [
  ...
  public validateRow(row: any, rowNum: number): RowError[] {
    ...
    if (this.importMode === "byStudentId") {
      ...
      if (this.isBlank(row["班級"]))
        errorMap.set("班級", ["匯入模式為班級座號，班級欄必填"]);
    ...
    if (!matchedStudent) {
      if (this.role === "班導師") {
        const key = this.importMode === "byStudentId" ? "學號" : "座號";
        errorMap.set(key, ["此學生本班學生，無法匯入"]);
      } else if (
        this.importMode === "byStudentId" &&
        !this.isSpecialStudentFn(matchedStudent)
      ) {
        errorMap.set("學號", ["此學生本班學生，無法匯入"]);
      } else if (this.role === "管理者") {
        const key = this.importMode === "byStudentId" ? "學號" : "座號";
        errorMap.set(key, ["非本校系統學生，無法匯入"]);
      }
    }
  ...
  private formatErrors(
    ...
    errorMap.forEach(function (messages, column) {
      const rawValue =
        row && typeof row[column] !== "undefined" ? row[column] : null;
      const valueText =
